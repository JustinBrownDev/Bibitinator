...
namespace Bibitinator
{
    public partial class BibiteEditor : Form
    {
        ...
        {
            propertiesTree.Nodes.Add(createNode(bibCol)); //------------------- this starts func for populating json properties tree
            ...
            buildTextboxes();                                   //------------- start function to setup genes editor
            Trace();                                            //------------- start function to trace inputs and outputs
        ...
        public TreeNode createNode(BibiteCollection col) //BUILD JSON PROPERTIES TREE VIEW
        {
        ...
        public void buildTextboxes() //BUILD GENES EDITOR
        {
            //Tag ref:
        ...
        private void synapseDelete(object sender, EventArgs e)
        {
            ...
            Trace();    //---------------------------------------------------- Refresh the brain tree without the deleted synapse
            BrainEditorPanel.Controls.Remove(((Button)sender).Parent); //----- Remove the panel from the synapse editor list
        }
        
        private void Trace() //TRACE SYNAPSE CONNECTIONS AND CREATE A TREE VIEW FROM OUTPUTS TO ITS INPUTS
        {
        ...
        private void saveButton_Click(object sender, EventArgs e) //Genes Save Button
        {
            Cursor = Cursors.WaitCursor;                                  
            if (File.Exists(bibCol.extractFolder + bibCol.name))           //- delete original file
            {
                File.Delete(bibCol.extractFolder + bibCol.name);
            }
            string json = ReplaceValuesInBibiteSettings(bibCol.json);      //- Save Genes
            bibCol.json = json;
            File.WriteAllText(bibCol.extractFolder + bibCol.name, json);   //- Write new file

            if (File.Exists(bibCol.extractFolder + bibCol.name))           //- Make sure its there
            {
        ...
        private void ConnectionsTreeView_AfterSelect(object sender, TreeViewEventArgs e) //Highlights synapse when you select a node in the brain tree
        {
            if (MovingLabel.Text == "")
            {
        ...
        private void addSynapse_Click(object sender, EventArgs e) //Adds a synapse to the brain editor
        {
        ...
        private void applyBrainChangesToModel() //Saves brain to DynRoot
        {
            ...
            foreach (Control con in BrainEditorPanel.Controls) //------------- Only Adds synapses and neurons referenced in brain editor
            ...
            bool complete = false;
            while (!complete)                                              //trim the synapses for only those that serve a function, this doesnt work entirely but its enough for the game to load
            {
            ...
            JArray validatedNodes = new JArray(); //-------------------------- create a list of nodes that are associated with the trimmed list of synapses
            foreach (JToken s in synapses)
            {
